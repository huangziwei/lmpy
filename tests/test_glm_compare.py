"""Phase 5 — ``lmpy.compare`` integration for ``glm``.
...
from lmpy import AIC, BIC, Binomial, Gamma, Gaussian, Poisson, anova, glm
...
def test_anova_glm(oid: str):
    ...
    df_lmpy = df["Df"].to_list()
    dev_lmpy = df["Deviance"].to_list()
    stat_lmpy = df[stat_col].to_list()
    p_lmpy = df[p_col].to_list()
    for i in range(len(o["df"])):
        df_R, dev_R = o["df"][i], o["deviance"][i]
        stat_R, p_R = o["stat"][i], o["pvalue"][i]
        if df_R is None:
            assert df_lmpy[i] is None and dev_lmpy[i] is None, \
                f"row {i}: expected None for first row"
            assert stat_lmpy[i] is None and p_lmpy[i] is None
            continue
        assert df_lmpy[i] == df_R, \
            f"row {i}: Df lmpy={df_lmpy[i]} R={df_R}"
        np.testing.assert_allclose(dev_lmpy[i], dev_R, atol=5e-3,
            err_msg=f"row {i}: Deviance lmpy={dev_lmpy[i]} R={dev_R}")
        np.testing.assert_allclose(stat_lmpy[i], stat_R, atol=5e-3,
            err_msg=f"row {i}: {stat_col} lmpy={stat_lmpy[i]} R={stat_R}")
        # p-value: meaningful values to 5e-3; tiny values just need to be tiny.
        if p_R > 1e-10:
            np.testing.assert_allclose(p_lmpy[i], p_R, atol=5e-3,
                err_msg=f"row {i}: {p_col} lmpy={p_lmpy[i]} R={p_R}")
        else:
            assert p_lmpy[i] < 1e-6, \
                f"row {i}: {p_col} lmpy={p_lmpy[i]} should be tiny like R={p_R}"

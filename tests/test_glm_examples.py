"""R-oracle parity tests for ``lmpy.glm``.

Each test pins ``lmpy.glm(...)`` outputs against the corresponding
...
# rebuild the lmpy fit corresponding to its R-side oracle. Centralizing this
...
def _build_menarche(link: str):
    # cbind(Menarche, Total - Menarche) ~ Age. lmpy's parser doesn't yet
...
def _allclose(actual, expected, *, atol, rtol=0.0, name=""):
    np.testing.assert_allclose(
        ...
        err_msg=f"{name}: lmpy={actual} R={expected}",
    )
...
def test_glm_core_fit(oid: str):
    ...
    # IRLS iter count: lmpy must not blow up vs R, but converging faster is
    ...
    # needs 11 iters, while lmpy's default init lands near the optimum and
    # converges in 4. Allow lmpy ≤ R+2.
    assert m.iter <= o["iter"] + 2, f"iter: lmpy={m.iter} R={o['iter']}"
...
def test_glm_inference(oid: str):
    ...
    assert m._test_kind == expected_kind, \
        f"test kind: lmpy={m._test_kind} R={expected_kind}"
    ...
    p_lmpy = np.asarray(m.p_values.row(0), dtype=float)
    p_R = np.asarray(o["p_value"])
    mask_meaningful = p_R > 1e-10
    if mask_meaningful.any():
        _allclose(p_lmpy[mask_meaningful], p_R[mask_meaningful],
                  atol=5e-3, name="p_value(meaningful)")
    # Tiny p-values: just assert lmpy is also tiny.
    if (~mask_meaningful).any():
        assert np.all(p_lmpy[~mask_meaningful] < 1e-6), \
            f"p-value(tiny): lmpy={p_lmpy} R={p_R}"
...
def test_glm_aic_bic_loglik(oid: str):
    ...
    assert m.npar == o["loglik_df"], \
        f"npar (= logLik$df): lmpy={m.npar} R={o['loglik_df']}"
